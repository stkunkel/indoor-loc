...
int ssTwrInit(void) {
	...
	while (1) {
		...
		if (status_reg & SYS_STATUS_RXFCG) {
			...
			myprintf("Errors occured. Clearing up\r\n");
			dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_ALL_RX_ERR);
		}

		/* Execute a delay between ranging exchanges. */
		//printf("Delay %dms\r\n",RNG_DELAY_MS);
		//while (waitAfterReceive() > 0)...
		// toggle led
		toggleLed(UWB_MASK);

		//update antenna (1 - decr, 2 - incr)
		// update antenna
		if (getButtonStatus() & 1) {
		    ...
		    myprintf("Decreased antenna delay to 0x%x\r\n",tx_delay);
		}
		if (getButtonStatus() & 2) {
		    ...
		    dwt_setrxantennadelay(rx_delay);
		    dwt_settxantennadelay(tx_delay);
		    myprintf("Increased antenna delay to 0x%x\r\n",tx_delay);
		}
	}
}

/*
 * Receive and get Distance
 * Out: Pointer to distance variable
 * Returns 0 if distance has been computed, 1 if nothing has been received yet, and -1 if there was an error
 */
int getDistance(double* p_distance){
	if (!((status_reg = dwt_read32bitreg(SYS_STATUS_ID)) & (SYS_STATUS_RXFCG | SYS_STATUS_ALL_RX_ERR))){
		return 1;
	} else {
		if (SYS_STATUS_RXRFTO & status_reg)
			myprintf("RX timeout\r\n");
...
int prepareSsTwrInit() {
	...
	myprintf("OTP   6: 0x%x\r\n", otpVal[6]);
	myprintf("OTP   7: 0x%x\r\n", otpVal[7]);
	myprintf("OTP x16: 0x%x\r\n", otpVal[0x16]);
	myprintf("OTP x17: 0x%x\r\n", otpVal[0x17]);

	/* Apply default antenna delay value. See NOTE 2 below. */
	myprintf(
			"antenna delays: default TX: %d, default RX: %d, evk 16m: %d, evk 64m: %d\r\n",
 ...
 * Returns difference btw specified delay and wait time
 */
int waitAfterReceive() {
	...
	//End of waiting time reached
	if (cnt == (RNG_DELAY_MS * IMU_SAMPLE_FREQ / 1000)) {
		cnt = 0;
		return cnt;
	} else {
		return ((RNG_DELAY_MS * IMU_SAMPLE_FREQ / 1000) - cnt);
	}
}

/*
 * Increase Antenna Delay
 */
