...
function angle_out = kalman_angle(angle_in, rate, stddev_angle, stddev_rate, delta_t)
  ...
  # Go through samples
  for i=2:size(angle_in)
  
    # Rememver previous error covariance
    P_prev = P;

    # Prediction Update
    angle_est = A*angle_out(i-1) + B*rate(i);
    P = A*P_prev*transpose(A) + Q;

    # Measurement Update
    K = P_prev*transpose(H)*inv(H*P_prev*transpose(H));
    angle_out(i) = angle_est + K*(angle - H*angle_est);
...
for i = 1:length(gx)
  ...
  if (acc_mag != 0)
	acc_norm = [(ax(i) / acc_norm_sc); (ay(i) / acc_norm_sc); (az(i) / acc_norm_sc)];
  else 
